...
export function calculateAmount(targetDate: Dayjs, pool: PPLPool, events: PPLEvent[]) {
    ...
    const gross_accrued_hours = elapsedPeriods(pool.startDate, targetDate, pool.period) * pool.amount
    ...
    if (event_hours.length > 0) {
        const total_event_hours = event_hours.reduce((acc, current) => acc + current)
