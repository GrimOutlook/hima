...
const PoolListing: React.FC<PoolListingProps> = ({className, pool}) => {
    ...
    let amount = calculateAmount(dayjs(), pool, events.filter((event) => event.pool == pool.id));
    ...
    return (
        <>
            ...
            hover:drop-shadow-lg p-2`}>
                ...
                onClick={() => {setIsActionsPanelOpen(!isActionsPanelOpen); setIsDetailsPanelOpen(false)}}>
                    {/* <!-- Hours --> */}
                    ...
                    {/* <!-- Name --> */}
                    <div className="text-2xl h-full w-full flex-3/4 line-clamp-1 place-self-center font-bold">{pool.name}</div>
                </div>
                {isDetailsPanelOpen && (
                    <div className={`relative border-t-black/10 border-t-2 w-full text-lg flex flex-col`}>
                        ...
                        <div className='text-md font-normal w-full text-center p-2'>
                            ...
                            <span className='m-1 text-xl font-semibold'>{pool.startDate.format("YYYY/MM/DD")}</span>
                ...
                {isActionsPanelOpen && (
                    <div className={`relative border-t-black/10 border-t-2 flex justify-between`}>
                        ...
                        <ChevronDownIcon className={`${isDetailsPanelOpen && 'rotate-180'} stroke-black size-7 p-0.5 rounded-full hover:bg-black/10 hover:cursor-pointer mt-2.5`}
